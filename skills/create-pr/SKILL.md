...
2. If the project contains Python files, run `ruff check` and `ruff format --check` in the repo root. Fix any errors before proceeding. If ruff is not installed, skip this step.
3. Run `git branch --show-current` to get the branch name
4. Run `git log main..HEAD --oneline` and `git diff main...HEAD` to understand what changed
5. Derive a clear, human-readable PR title from both the branch name and the actual changes — do not just restate the branch name verbatim; synthesize a title that captures the intent
6. Push the branch with `git push -u origin <branch>` if not already pushed
7. Open the PR with `gh pr create --base main --title "..." --body "..."` using the standard body format below
